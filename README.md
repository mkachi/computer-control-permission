...
래퍼가 최근 30초 활동 기준을 고정하고 승인 프로그램이 종료될 때까지 기다린 뒤,
검증된 JSON 결과와 종료 코드를 반환합니다. 강제로 창을 띄우는 모드는 제공하지
않으며, 정책 인자와 결과 파일은 래퍼 내부에서만 관리합니다. 에이전트가 파일 생성
시점을 추측하거나 같은 승인을 다시 요청할 필요가 없습니다. 도구가 실행 중 세션
ID를 반환하면 새 요청을 만들지 않고 같은 실행을 계속 기다립니다.
